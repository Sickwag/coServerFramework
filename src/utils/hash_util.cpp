...
std::string base64encode(const std::string& data) { return base64encode(data.c_str(), data.size()); }
...
std::string md5sum(const std::string& data) { return md5sum(data.c_str(), data.size()); }
...
std::string sha0sum(const std::string& data) { return sha0sum(data.c_str(), data.length()); }
...
std::string hmac_sha256(const std::string& text, const std::string& key) {
	return hmac<SHA256_CTX, &SHA256_Init, &SHA256_Update, &SHA256_Final, SHA256_CBLOCK, SHA256_DIGEST_LENGTH>(
		text, key);
}
