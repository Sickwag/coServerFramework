...
namespace {
ConfigVar<uint64_t>::ptr g_tcpServerReadTimeout = Config::lookup("tcp_server.read_timeout",
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs, std::vector<Address::ptr>& fails, bool ssl) {
	...
	for(auto& addr : addrs) {
		...
		if(!sock->bind(addr)) {
			AZZATO_LOG_ERROR(AZZATO_LOG_ROOT())
				<< "bind fail errno=" << errno << " errstr=" << std::strerror(errno) << " addr=["
				<< addr->toString() << "]";
		...
		if(!sock->listen()) {
			AZZATO_LOG_ERROR(AZZATO_LOG_ROOT())
				<< "listen fail errno=" << errno << " errstr=" << std::strerror(errno) << " addr=["
				<< addr->toString() << "]";
			fails.push_back(addr);
			continue;
		}
	...
	for(auto& sock : _socks) {
		AZZATO_LOG_INFO(AZZATO_LOG_ROOT())
			<< "type=" << _type << " name=" << _name << " ssl=" << _ssl << " server bind success: " << *sock;
	}
	return true;
}
...
void TcpServer::startAccept(Socket::ptr sock) {
	while(!_isStop) {
		...
		if(client) {
			...
			AZZATO_LOG_ERROR(AZZATO_LOG_ROOT())
				<< "accept errno=" << errno << " errstr=" << std::strerror(errno);
		}
	}
}
