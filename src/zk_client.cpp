...
bool ZKClient::init(const std::string& hosts, int recv_timeout, watcher_callback cb, log_callback lcb) {
	...
	_watcherCb	 = std::bind(
		  cb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, shared_from_this());
	_logCb	= lcb;
	_handle = zookeeper_init2(hosts.c_str(), &ZKClient::OnWatcher, _recvTimeout, nullptr, this, 0, lcb);
	return _handle != nullptr;
}

int32_t ZKClient::setServers(const std::string& hosts) {
	auto rt = zoo_set_servers(_handle, hosts.c_str());
	if(rt == 0) {
		_hosts = hosts;
	}
	return rt;
...
int32_t
ZKClient::getChildren(const std::string& path, std::vector<std::string>& val, bool watch, Stat* stat) {
...
int32_t ZKClient::getState() { return zoo_state(_handle); }
