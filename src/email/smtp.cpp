...
SmtpClient::SmtpClient(Socket::ptr sock)
	: azzato::SocketStream(sock) {}

SmtpClient::ptr SmtpClient::Create(const std::string& host, uint32_t port, bool ssl) {
	azzato::IPAddress::ptr addr = azzato::Address::lookupAnyIPAddress(host);
	if(!addr) {
		AZZATO_LOG_ERROR(g_logger) << "invalid smtp server: " << host << ":" << port << " ssl=" << ssl;
		return nullptr;
	}
	addr->setPort(port);
	Socket::ptr sock;
	if(ssl) {
		sock = azzato::SSLSocket::createTcp(addr);
	} else {
		sock = azzato::Socket::createTcp(addr);
	}
	if(!sock->connect(addr)) {
		AZZATO_LOG_ERROR(g_logger) << "connect smtp server: " << host << ":" << port << " ssl=" << ssl
								   << " fail";
		return nullptr;
	}
	std::string buf;
	...
	buf.resize(len);
	if(azzato::TypeUtil::atoi(buf) != 220) {
		return nullptr;
	}
	rt->_host = host;
	return rt;
...
SmtpResult::ptr SmtpClient::doCmd(const std::string& cmd, bool debug) {
	...
	int code = azzato::TypeUtil::atoi(buf);
	if(code >= 400) {
		return std::make_shared<SmtpResult>(code, azzato::replace(buf.substr(buf.find(' ') + 1), "\r\n", ""));
	}
	return nullptr;
}
...
#define DO_CMD()                \
	...
	std::string		cmd = "HELO " + _host + "\r\n";
	DO_CMD();
	if(!_authed && !email->getFromEMailAddress().empty()) {
		cmd = "AUTH LOGIN\r\n";
		DO_CMD();
		auto pos = email->getFromEMailAddress().find('@');
		cmd		 = azzato::base64encode(email->getFromEMailAddress().substr(0, pos)) + "\r\n";
		DO_CMD();
		cmd = azzato::base64encode(email->getFromEMailPasswd()) + "\r\n";
		DO_CMD();
...
#define XX(fun)                                \
	...
	std::string boundary;
	if(!entitys.empty()) {
		boundary = azzato::random_string(16);
...
std::string SmtpClient::getDebugInfo() { return _ss.str(); }
