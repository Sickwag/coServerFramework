...
int SQLite3::getErrno() { return sqlite3_errcode(_db); }

std::string SQLite3::getErrStr() { return sqlite3_errmsg(_db); }
...
int SQLite3::execute(const std::string& sql) { return sqlite3_exec(_db, sql.c_str(), 0, 0, 0); }
...
int64_t SQLite3Stmt::getLastInsertId() { return _db->getLastInsertId(); }
...
int SQLite3Stmt::getErrno() { return _db->getErrno(); }

std::string SQLite3Stmt::getErrStr() { return _db->getErrStr(); }

int SQLite3Stmt::bind(int idx, int32_t value) { return sqlite3_bind_int(_stmt, idx, value); }

int SQLite3Stmt::bind(int idx, uint32_t value) { return sqlite3_bind_int(_stmt, idx, value); }

int SQLite3Stmt::bind(int idx, double value) { return sqlite3_bind_double(_stmt, idx, value); }

int SQLite3Stmt::bind(int idx, int64_t value) { return sqlite3_bind_int64(_stmt, idx, value); }

int SQLite3Stmt::bind(int idx, uint64_t value) { return sqlite3_bind_int64(_stmt, idx, value); }

int SQLite3Stmt::bind(int idx, const char* value, Type type) {
	return sqlite3_bind_text(
		_stmt, idx, value, strlen(value), type == COPY ? SQLITE_TRANSIENT : SQLITE_STATIC);
}
...
int SQLite3Stmt::bind(int idx, const std::string& value, Type type) {
	return sqlite3_bind_text(
		_stmt, idx, value.c_str(), value.size(), type == COPY ? SQLITE_TRANSIENT : SQLITE_STATIC);
}

int SQLite3Stmt::bind(int idx) { return sqlite3_bind_null(_stmt, idx); }
...
int SQLite3Stmt::step() { return sqlite3_step(_stmt); }

int SQLite3Stmt::reset() { return sqlite3_reset(_stmt); }
...
int SQLite3Data::getDataCount() { return sqlite3_data_count(_stmt->_stmt); }

int SQLite3Data::getColumnCount() { return sqlite3_column_count(_stmt->_stmt); }

int SQLite3Data::getColumnBytes(int idx) { return sqlite3_column_bytes(_stmt->_stmt, idx); }

int SQLite3Data::getColumnType(int idx) { return sqlite3_column_type(_stmt->_stmt, idx); }
...
int32_t SQLite3Data::getInt32(int idx) { return sqlite3_column_int(_stmt->_stmt, idx); }

uint32_t SQLite3Data::getUint32(int idx) { return getInt32(idx); }

int64_t SQLite3Data::getInt64(int idx) { return sqlite3_column_int64(_stmt->_stmt, idx); }

uint64_t SQLite3Data::getUint64(int idx) { return getInt64(idx); }

float SQLite3Data::getFloat(int idx) { return getDouble(idx); }

double SQLite3Data::getDouble(int idx) { return sqlite3_column_double(_stmt->_stmt, idx); }
...
SQLite3Transaction::~SQLite3Transaction() {
	if(_status == 1) {
		...
		if(_status == 1) {
			AZZATO_LOG_ERROR(systemLogger) << _db << " auto_commit=" << _autoCommit << " fail";
		}
	}
}

int SQLite3Transaction::execute(const std::string& sql) { return _db->execute(sql); }

int64_t SQLite3Transaction::getLastInsertId() { return _db->getLastInsertId(); }
...
SQLite3::ptr SQLite3Manager::get(const std::string& name) {
	...
	if(path.empty()) {
		AZZATO_LOG_ERROR(systemLogger) << "open db name=" << name << " path is null";
		return nullptr;
	}
	...
	if(sqlite3_open_v2(path.c_str(), &db, SQLite3::CREATE | SQLite3::READWRITE, nullptr)) {
		AZZATO_LOG_ERROR(systemLogger) << "open db name=" << name << " path=" << path << " fail";
		return nullptr;
	}
	...
	if(!sql.empty()) {
		if(rt->execute(sql)) {
			AZZATO_LOG_ERROR(systemLogger)
				<< "execute sql=" << sql << " errno=" << rt->getErrno() << " errstr=" << rt->getErrStr();
			delete rt;
			return nullptr;
		}
	}
...
int SQLite3Manager::execute(const std::string& name, const std::string& sql) {
	auto conn = get(name);
	if(!conn) {
		AZZATO_LOG_ERROR(systemLogger) << "SQLite3Manager::execute, get(" << name << ") fail, sql=" << sql;
		return -1;
	}
...
ISQLData::ptr SQLite3Manager::query(const std::string& name, const std::string& sql) {
	auto conn = get(name);
	if(!conn) {
		AZZATO_LOG_ERROR(systemLogger) << "SQLite3Manager::query, get(" << name << ") fail, sql=" << sql;
		return nullptr;
	}
...
SQLite3Transaction::ptr SQLite3Manager::openTransaction(const std::string& name, bool auto_commit) {
	auto conn = get(name);
	if(!conn) {
		AZZATO_LOG_ERROR(systemLogger) << "SQLite3Manager::openTransaction, get(" << name << ") fail";
		return nullptr;
	}
