...
std::string Column::getDefaultValueString() {
	...
	return "azzato::Str2Time(\"" + _default + "\")";
}
...
bool Column::init(const tinyxml2::XMLElement& node) {
	if(!node.Attribute("name")) {
		AZZATO_LOG_ERROR(g_logger) << "column name not exists";
		return false;
	}
	_name = node.Attribute("name");

	if(!node.Attribute("type")) {
		AZZATO_LOG_ERROR(g_logger) << "column name=" << _name << " type is null";
		return false;
	}
	_type  = node.Attribute("type");
	_dtype = ParseType(_type);
	if(_dtype == TYPE_NULL) {
		AZZATO_LOG_ERROR(g_logger) << "column name=" << _name << " type=" << _type << " type is invalid";
		return false;
	}
	...
	if(node.Attribute("length")) {
		_length = node.IntAttribute("length");
	} else {
		_length = 0;
	}

	_autoIncrement = node.BoolAttribute("auto_increment", false);
	return true;
}
...
std::string Column::getGetFunDefine() const {
	std::stringstream ss;
	ss << "const " << TypeToString(_dtype) << "& " << GetAsGetFunName(_name) << "() { return "
	   << GetAsMemberName(_name) << "; }" << std::endl;
	return ss.str();
}

std::string Column::getSetFunDefine() const {
	std::stringstream ss;
	ss << "void " << GetAsSetFunName(_name) << "(const " << TypeToString(_dtype) << "& v);" << std::endl;
	return ss.str();
}
