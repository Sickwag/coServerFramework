...
std::string GetAsVariable(const std::string& v) { return azzato::toLower(v); }

std::string GetAsClassName(const std::string& v) {
	auto			  vs = azzato::split(v, '_');
	std::stringstream ss;
	for(auto& i : vs) {
		i[0] = toupper(i[0]);
		ss << i;
...
std::string XmlToString(const tinyxml2::XMLNode& node) { return ""; }

std::string GetAsDefineMacro(const std::string& v) {
	std::string tmp = azzato::replace(v, '.', '_');
	tmp				= azzato::toUpper(tmp);
	return "__" + tmp + "__";
}
