...
class Roaring {
  public:
	...
	Roaring(const Roaring& r) {
		bool is_ok = ra_copy(&r.roaring.high_low_container,
							 &roaring.high_low_container,
	 ...
	 */
	void addRange(const uint64_t x, const uint64_t y) { return roaring_bitmap_add_range(&roaring, x, y); }

	/**
	 * Add value n_args from pointer vals
	 *
	 */
	void addMany(size_t n_args, const uint32_t* vals) { roaring_bitmap_add_many(&roaring, n_args, vals); }
	 ...
	 */
	void iterate(roaring_iterator iterator, void* ptr) const { roaring_iterate(&roaring, iterator, ptr); }
	 ...
	 *
	 */
	bool intersect(const Roaring& r) const { return roaring_bitmap_intersect(&roaring, &r.roaring); }
...
inline RoaringSetBitForwardIterator Roaring::begin() const { return RoaringSetBitForwardIterator(*this); }
...
inline RoaringSetBitReverseIterator Roaring::rbegin() const { return RoaringSetBitReverseIterator(*this); }
...
class Roaring64Map {
  public:
	...
	uint64_t cardinality() const {
		...
		return std::accumulate(roarings.cbegin(),
							   roarings.cend(),
							   (uint64_t)0,
	...
	bool isEmpty() const {
		return std::all_of(
			roarings.cbegin(), roarings.cend(), [](const std::pair<uint32_t, Roaring>& map_entry) {
	...
	void toUint64Array(uint64_t* ans) const {
		...
		(void)std::accumulate(roarings.cbegin(),
							  roarings.cend(),
							  ans,
							  [](uint64_t* previous, const std::pair<uint32_t, Roaring>& map_entry) {
	...
	bool removeRunCompression() {
		return std::accumulate(roarings.begin(),
							   roarings.end(),
							   false,
	...
	void iterate(roaring_iterator64 iterator, void* ptr) const {
		std::for_each(roarings.begin(), roarings.cend(), [=](const std::pair<uint32_t, Roaring>& map_entry) {
	...
	void setCopyOnWrite(bool val) {
		if(copyOnWrite == val)
			return;
		copyOnWrite = val;
		std::for_each(roarings.begin(), roarings.end(), [=](std::pair<const uint32_t, Roaring>& map_entry) {
	...
	void printf() const {
		if(!isEmpty()) {
			...
			map_iter->second.iterate(
				...
				(void*)&outer_iter_data);
			std::for_each(++map_iter, roarings.cend(), [](const std::pair<uint32_t, Roaring>& map_entry) {
